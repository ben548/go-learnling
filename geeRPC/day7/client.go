...
func dialTimeout(f newClientFunc, network, addr string, opts ...*Option) (client *Client, err error) {
	...
	conn, err := net.DialTimeout(network, addr, options.ConnectionTimeout)
	if err != nil {
		return nil, err
	}
	defer func() {
	...
	case <-time.After(options.ConnectionTimeout):
		return nil, fmt.Errorf("rpc client: connect timeout: expect within %s", options.ConnectionTimeout)
	case result := <-ch:
		return result.client, result.err
	}
}
